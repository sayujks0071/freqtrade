...
def main():
    ...
    params = (target_date.strftime("%Y-%m-%d %H:%M:%S"), end_date.strftime("%Y-%m-%d %H:%M:%S"))
    ...
    if total_trades == 0:
        ...
        avg_profit_pct = sum((t['close_profit'] or 0) for t in trades) / total_trades if total_trades > 0 else 0
...
"""
    ...
    # Write report
    outfile = os.path.join(args.out, f"daily_summary_{target_date.strftime('%Y%m%d')}.md")

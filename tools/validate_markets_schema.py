...
def main():
    ...
    required_fields = [
        ...
        "quote",
        "active",
    ]  # 'info' is usually there but minimal check
    symbols = set()
    active_count = 0
    ...
    for m in markets:
        ...
        for field in required_fields:
            if field not in m:
                fail(
                    f"Missing field '{field}' in market: {m.get('symbol', 'UNKNOWN')}", report_file
                )
    ...
    if prev_markets_file and os.path.exists(prev_markets_file):
        try:
            ...
            if removal_ratio > MAX_REMOVAL_RATIO:
                fail(
                    f"Drift Safety Gate Triggered! Removed ratio {removal_ratio:.2%} > {MAX_REMOVAL_RATIO}",
                    report_file,
                )

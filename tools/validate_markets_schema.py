...
def is_eligible(market):
    ...
    if FILTER_MODE == 'perps_usdt':
        ...
        return market.get('future', False) or market.get('swap', False) or market.get('contract', False)
...
def main():
    ...
    parser.add_argument("--candidate-whitelist", required=False, help="Path to candidate whitelist JSON (output of this script if valid)")
    parser.add_argument("--prev-whitelist", required=False, help="Path to previous whitelist JSON for drift check")
    ...
    if args.prev_whitelist and os.path.exists(args.prev_whitelist):
        ...
        if prev_data:
            ...
            if removed:
                report_lines.append("\n### Removed Pairs")
                report_lines.append(", ".join(list(removed)[:20]) + ("..." if len(removed)>20 else ""))
            if added:
                report_lines.append("\n### Added Pairs")
                report_lines.append(", ".join(list(added)[:20]) + ("..." if len(added)>20 else ""))

    # 4. Generate Output Whitelist (if requested)
    if args.candidate_whitelist and drift_status == "PASS":
        out_data = {
            "exchange": {
                "pair_whitelist": sorted(list(valid_symbols))
            }
        }

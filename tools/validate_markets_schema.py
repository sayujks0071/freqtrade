...
def _check_numeric_limits(m, symbol, errors):
...
def _check_volume_strict(m, symbol, errors):
...
def validate_volume(m, symbol, errors):
    ...
    # Check 'limits'
    _check_numeric_limits(m, symbol, errors)

    # Optionally filter out near-zero volume markets if volume is available
    _check_volume_strict(m, symbol, errors)
...
def validate_drift(current_whitelist_symbols, prev_whitelist_path):
    ...
    try:
        with prev_path_obj.open() as f:
            ...
            if (
                ...
                return (
                    [f"Unknown format in previous whitelist {prev_whitelist_path}"],
                    None,
                    None,
                )
    except Exception as e:
    ...
    if removal_ratio > MAX_REMOVAL_RATIO:
        drift_errors.append(
            f"Large delist drift: {removal_ratio:.2f} > {MAX_REMOVAL_RATIO} (-{len(removed)} pairs)"
        )

    return drift_errors, drift_info, prev_symbols


def check_env_sanity(data):
    # 2. Env Sanity Check
...
def generate_report_content(args, status, all_symbols, current_whitelist, drift_info, all_errors):
    # Generate Report
    report_lines = [
...
def main():
    ...
    # 2. Env Sanity Check
    check_env_sanity(data)

    # 3. Simulate Whitelist Generation
    current_whitelist = set(filter_markets(data))

    # 4. Drift Check
    drift_errors, drift_info, _ = validate_drift(current_whitelist, args.prev_whitelist)
    ...
    report_content = generate_report_content(
        args, status, all_symbols, current_whitelist, drift_info, all_errors
    )

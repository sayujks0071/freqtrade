    ...
    if report_path and report_lines is not None:
        try:
            ...
            if len(report_lines) > 2 and "Status:" not in report_lines[2]:
                 report_lines.insert(2, f"Status: **FAIL**")
...
def validate_market_structure(i, m, errors):
    ...
    if not any(k in m for k in ["type", "contract", "future", "spot", "swap", "linear", "inverse"]):
...
def validate_volume(m, symbol, errors):
    # Volume check
    for field in ["volume", "limits"]:
        if field == "volume" and field in m:
             vol = m.get("volume")
             if vol is not None:
...
def main():
    ...
    try:
        ...
        fail(f"Invalid JSON: {e}", args.out_report, report_lines)
    ...
    if len(data) < MIN_MARKETS:
        fail(f"Market count {len(data)} < MIN_MARKETS ({MIN_MARKETS})", args.out_report, report_lines)
    ...
    # We lack metadata in standard list-markets dump to strictly verify env.

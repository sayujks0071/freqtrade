...
class StrategyScout:
    ...
    def filter_and_score(self):
        ...
        for repo in self.candidates:
            ...
            score = self._calculate_license_score(license_data, full_name)
            if license_data:
                license_name = license_data.get("name", "Unknown")

            # 2. Recency
            recency_score, age_days = self._calculate_recency_score(pushed_at)
            score += recency_score
    ...
    def _calculate_license_score(self, license_data, full_name):
        score = 0
        if license_data and license_data.get("key") != "other":
            score += 5  # Clear license
        elif license_data and license_data.get("key") == "other":
            score += 1
        elif full_name not in KNOWN_SOURCES:
            return 0  # No license and not known source
        return score

    def _calculate_recency_score(self, pushed_at):
        score = 0
        age_days = 9999
        if pushed_at:

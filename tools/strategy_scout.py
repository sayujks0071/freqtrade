...
def parse_args():
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("user_data/reports"), help="Output directory")
    parser.add_argument("--vendor", action="store_true", help="Vendor strategies to user_data/strategies_vendor")
...
def analyze_strategy(content):
    ...
    indicators = []
    if "talib" in content: indicators.append("talib")
    if "qtpylib" in content: indicators.append("qtpylib")
    if "pandas_ta" in content: indicators.append("pandas_ta")
    if "technical" in content: indicators.append("technical")
    if indicators:
...
def main():
    ...
    # Add known sources if not present
    # (Simplified logic: usually search finds them if updated, but force add if needed)
    ...
    for repo in repos:
        ...
        if not license and name not in KNOWN_SOURCES:
            logger.warning(f"Skipping {name}: No license")
            continue

        # Check Recency
        updated_at = datetime.fromisoformat(repo["updated_at"].replace("Z", "+00:00"))
        if datetime.now(timezone.utc) - updated_at > timedelta(days=365):
            logger.warning(f"Skipping {name}: Stale (>1 year)")
            continue

        # Scan Files
        contents = get_repo_content(name, "user_data/strategies" if "freqtrade" in name else "", args.token)
        if not contents:
            contents = get_repo_content(name, "", args.token)
        ...
        # Limit deep inspection
        for f in strategy_files[:3]:
            file_data = get_repo_content(name, f["path"], args.token)
            if not file_data:
                continue
            ...
            if score > 0:
                results.append({
                    ...
                    "notes": notes,
                    "raw_content": content if args.vendor else None
                })

    # Generate Report
    results.sort(key=lambda x: x["score"], reverse=True)

    report_file = args.output_dir / f"strategy_shortlist_{datetime.now(timezone.utc).strftime('%Y%m%d')}.md"

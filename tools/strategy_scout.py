...
class StrategyScout:
    ...
    def filter_and_score(self):
        ...
        for repo in self.candidates:
            ...
            # 1. License Check
            license_name = self._calculate_license_score(license_data, full_name, repo)
            if license_name is None:
                continue
            score += repo.get("scout_score", 0)  # Add score from helper if any

            # 2. Recency
            score += self._calculate_recency_score(pushed_at, repo)
    ...
    def _calculate_license_score(self, license_data, full_name, repo):
        license_name = "Unknown"
        if license_data and license_data.get("key") != "other":
            license_name = license_data.get("name", "Unknown")
            repo["scout_score"] = 5  # Clear license
    ...
    def _calculate_recency_score(self, pushed_at, repo):
        score = 0
        if pushed_at:
    ...
    def deep_inspect(self, limit=15):
        ...
        # Re-sort to ensure we process highest scores first

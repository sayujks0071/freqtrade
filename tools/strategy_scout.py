...
Automatically discovers and shortlists the best open-source Python
...
class StrategyScout:
    ...
    def _add_known_sources(self, found_repos: dict[str, Any]):
        for source in KNOWN_SOURCES:
            if source not in found_repos:
                ...
                try:
                    resp = self.session.get(
                        f"{GITHUB_API_URL}/repos/{source}", timeout=10
                    )
                    if resp.status_code == 200:
    ...
    def _find_strategies(self, full_name: str) -> tuple[list[Any], str | None]:
        strategies = []
        found_path = None
        # Try specific paths
        ...
        for path in paths_to_check:
            try:
                ...
                # S110: Log exception
                logging.exception(f"Error checking path {path} for {full_name}")

        return strategies, found_path

    def _inspect_content(self, repo: dict[str, Any], content: str) -> None:
        # Check heuristics
    ...
    def _inspect_repo(self, repo: dict[str, Any]) -> None:
        full_name = repo["full_name"]
        print(f"Inspecting {full_name}...")

        strategies, found_path = self._find_strategies(full_name)

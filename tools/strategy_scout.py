...
def evaluate_repo(repo):
    ...
    if not license_info or license_info.get("key") not in ["mit", "apache-2.0", "bsd-3-clause", "gpl-3.0", "mpl-2.0"]:
...
def main():
    ...
    # Generate Report
    report_file = f"user_data/reports/strategy_shortlist_{datetime.now(timezone.utc).strftime('%Y%m%d')}.md"
    ...
    with open(report_file, 'w') as f:
        ...
        for i, c in enumerate(candidates[:20]): # Top 20
            ...
            f.write(f"| {i+1} | [{c['name']}]({c['url']}) | {c['score']} | {c['stars']} | {c['updated'][:10]} | {desc} |\n")

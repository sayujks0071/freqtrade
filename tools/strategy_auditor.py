...
class LogicVisitor(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node):
        if node.name in ["populate_entry_trend", "populate_exit_trend", "populate_entry_trend_short", "populate_exit_trend_short"]:
    ...
    def check_logic(self, node):
        ...
        if not has_comment:
            ...
            if not ast.get_docstring(node):
                self.errors.append(f"Function {node.name} at line {node.lineno} missing comments explaining market thesis")
            ...
            if isinstance(child, ast.Assign):
                for target in child.targets:
                    if isinstance(target, ast.Subscript): # df.loc[...]
                        ...
                        if condition_node:
                            if self.is_complex(condition_node):
                                self.errors.append(f"Complex inline condition in {node.name} at line {child.lineno}. Use named boolean variables.")

...
class Bandtastic(IStrategy):
    ...
    buy_trigger = CategoricalParameter(
        ["bb_lower1", "bb_lower2", "bb_lower3", "bb_lower4"], default="bb_lower1", space="buy"
    )
    ...
    sell_rsi_enabled = CategoricalParameter(
        [True, False], space="sell", optimize=True, default=False
    )
    sell_mfi_enabled = CategoricalParameter(
        [True, False], space="sell", optimize=True, default=True
    )
    sell_ema_enabled = CategoricalParameter(
        [True, False], space="sell", optimize=True, default=False
    )
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.buy_ema_enabled.value:
            conditions.append(
                dataframe[f"EMA_{self.buy_fastema.value}"]
                > dataframe[f"EMA_{self.buy_slowema.value}"]
            )
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.sell_ema_enabled.value:
            conditions.append(
                dataframe[f"EMA_{self.sell_fastema.value}"]
                < dataframe[f"EMA_{self.sell_slowema.value}"]
            )

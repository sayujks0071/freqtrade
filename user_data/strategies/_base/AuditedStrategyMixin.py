...
class AuditedStrategyMixin:
    ...
    def check_daily_loss_limit(self, max_loss_ratio: float) -> bool:
        ...
        try:
            ...
            total_profit_abs = sum(t.close_profit_abs for t in trades)

            # Assuming self.wallets is available in strategy
            # Use total_investment or available_capital if wallets not available?
            # self.wallets might be available. If not, we can't calculate ratio accurately.
            # Fallback to config 'stake_amount' * 'max_open_trades' as rough capital estimate if wallets fail?
            ...
            if loss_ratio < -max_loss_ratio:
                logger.warning(f"AUDIT_RISK | Daily loss limit hit! Loss: {loss_ratio:.2%}, Limit: {max_loss_ratio:.2%}")
                return False

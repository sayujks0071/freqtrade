...
-   [Symbol Mapping](user_data/reports/symbol_mapping_20260204.md)
-   [Freqtrade Documentation](https://www.freqtrade.io)

## Strategy Auditing & Logs

### How to choose pairs on Delta
**Never hand-type pairs blindly.**
1. Use the markets dump generated by `./scripts/validate_exchange.sh`.
2. Refer to `user_data/reports/symbol_mapping_*.md` for format examples.
3. Always use the full futures format: `BASE/QUOTE:SETTLE` (e.g. `BTC/USDT:USDT`).

### How to interpret logs
Strategies using the audit layer produce structured logs:
`AUDIT_SIGNAL | TIMESTAMP | PAIR | SIDE | REASON | SNAPSHOT`

- **TIMESTAMP**: UTC ISO-8601 time of the signal.
- **SIDE**: `long` or `short`.
- **REASON**: The specific condition tag that triggered the trade (e.g. `rsi_oversold`).
- **SNAPSHOT**: Key indicator values at the time of the signal (e.g. `{'rsi': 25, 'volume': 1000}`).

Check `user_data/logs/freqtrade.log` for these entries to verify decision logic.

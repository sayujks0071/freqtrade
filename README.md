...
All strategies are audited for:
- **Symbol Sanity**: Pairs must match Freqtrade futures format (`BASE/QUOTE:SETTLE`).
- **Audit Logs**: Every signal is logged with reason and timestamp.
- **Code Standards**: No giant one-liners, required metadata headers.

### How to choose pairs on Delta
Always pick pairs from the generated markets dump or the symbol mapping report.
**Never hand-type pairs blindly.**

See [Symbol Mapping Report](user_data/reports/symbol_mapping_*.md) (generated by `tools/generate_symbol_mapping.py`).
Common format: `BTC/USDT:USDT` (Linear), `BTC/USD:BTC` (Inverse).

### How to interpret logs
Audit logs are written to the standard Freqtrade log. Search for `AUDIT_SIGNAL`.

Format:
`AUDIT_SIGNAL | UTC_TIMESTAMP | PAIR | SIDE | REASON | SNAPSHOT`

Example:
`AUDIT_SIGNAL | 2023-10-27T10:00:00+00:00 | BTC/USDT:USDT | long | Signal Confirmed | {'rsi': 25.5, 'volume': 1500}`

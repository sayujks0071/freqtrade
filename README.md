...
-   [Symbol Mapping & Format](user_data/reports/symbol_mapping_example.md)
-   [Freqtrade Documentation](https://www.freqtrade.io)

## How to choose pairs on Delta

Always pick pairs from the markets dump generated by `./scripts/validate_exchange.sh`.
Never hand-type pairs blindly.
The format must be `BASE/QUOTE:SETTLE` (e.g. `BTC/USDT:USDT`).
See `user_data/reports/symbol_mapping_example.md` for details.

## How to interpret logs

Strategies using `AuditedStrategyMixin` will output audit logs in the following format:
```
AUDIT_SIGNAL | TIMESTAMP | PAIR | SIDE | REASON | CANDLE | INDICATORS
```
Example:
```
AUDIT_SIGNAL | 2023-10-27T10:00:00+00:00 | BTC/USDT:USDT | long | Signal Confirmed (RSI/Vol) | 2023-10-27 10:00:00 | {'rsi': 25.5, 'volume': 1500}
```
You can trace every trade decision back to these logs in `user_data/logs/freqtrade.log`.

...
The behavior of the market refresh validation can be tuned in `.env`:

| Variable | Default | Description |
|---|---|---|
| `MIN_MARKETS` | 20 | Minimum number of markets required in the dump. |
| `MAX_REMOVAL_RATIO` | 0.25 | Max fraction of whitelisted pairs allowed to disappear before failing (0.25 = 25%). |
| `STRICT_VOLUME` | false | If `true`, fails validation if quote volume < 1000 (if available). |
| `FILTER_MODE` | perps_usdt | Strategy for whitelist generation. Options: `perps_usdt`, `all_futures`, `allowlist_regex`. |
| `ALLOWLIST_REGEX` | .*USDT | Regex for `allowlist_regex` mode. |

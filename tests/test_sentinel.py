        ...
        with patch("scripts.sentinel.requests.Session") as mock_session_cls:
            ...
            with patch("scripts.sentinel.Sentinel.load_state", return_value=[]):
                ...
                with patch("scripts.sentinel.Sentinel.save_state"):
                    sentinel = Sentinel(mock_config, panic_sell=True, openclaw_url="http://openclaw", dry_run=False)
...
def test_trigger_emergency(mock_sentinel):
    with patch("scripts.sentinel.requests.post") as mock_post:
        ...
        mock_sentinel.session.get.return_value.json.return_value = [{"trade_id": 1, "pair": "ETH/USDT"}]
        mock_sentinel.session.post.return_value.status_code = 200

        mock_sentinel.trigger_emergency("Test Reason")

        # Alert sent
        mock_post.assert_called_with("http://openclaw", json={"message": "CRITICAL ALERT: Test Reason. Triggering Circuit Breaker.", "text": "CRITICAL ALERT: Test Reason. Triggering Circuit Breaker."})

        # Stop called
        mock_sentinel.session.post.assert_any_call("http://127.0.0.1:8080/api/v1/stop")

        # Panic sell called (since enabled in fixture)
        mock_sentinel.session.post.assert_any_call("http://127.0.0.1:8080/api/v1/forceexit", json={"tradeid": 1})

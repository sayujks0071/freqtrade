...
class TestCCXTExchange:
    ...
    def test_ccxt_fetch_mark_price_history(self, exchange_futures: EXCHANGE_FIXTURE_TYPE):
        ...
        if not (prev_mark_candles := mark_candles[mark_candles["date"] == prev_hour]).empty:
            assert prev_mark_candles.iloc[0]["open"] != 0.0
        if not (this_mark_candles := mark_candles[mark_candles["date"] == this_hour]).empty:
            assert this_mark_candles.iloc[0]["open"] != 0.0

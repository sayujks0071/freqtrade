...
class TestRegimeSwitcher(unittest.TestCase):
    def test_calculate_indicators(self):
        ...
        # Check for ADX column (pandas_ta names might vary, logic handles it but checks existence)
    ...
    def test_determine_regime_bull(self):
        row = {"close": 50000, "EMA_200": 40000, "ADX_14": 30, "volatility_spike": False}
        _regime, strategy = determine_regime(row)
        self.assertEqual(strategy, STRATEGY_BULL)

    def test_determine_regime_sideways(self):
        row = {"close": 40000, "EMA_200": 40000, "ADX_14": 15, "volatility_spike": False}
        _regime, strategy = determine_regime(row)
        self.assertEqual(strategy, STRATEGY_SIDEWAYS)

    def test_determine_regime_volatile(self):
        row = {"close": 30000, "EMA_200": 40000, "ADX_14": 40, "volatility_spike": True}
        _regime, strategy = determine_regime(row)

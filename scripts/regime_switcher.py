...
def detect_regime(df):
    ...
    # Else default to BollingerRSI or keep previous (here defaulting to BollingerRSI for safety/sideways assumption if ADX is between 20-25 and Price > EMA200, or other edge cases)

    return regime, strategy, close, ema200, adx

def update_config(strategy_name):
    config_path = 'user_data/configs/config_production.json'
...
def log_decision(regime, strategy, close, ema, adx):
    log_path = 'regime_log.md'
    timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    log_entry = f"| {timestamp} | {regime} | {strategy} | Price: {close:.2f} | EMA200: {ema:.2f} | ADX: {adx:.2f} |\n"

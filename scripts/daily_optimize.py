...
def establish_baseline():
    """Establish baseline backtest results."""
...
def run_hyperopt_process(worst_strategy):
    """Run hyperopt optimization for the strategy."""
    ...
    if new_params:
        ...
        return strategy_json, backup_json
    else:
...
def evaluate_results(worst_strategy, current_sharpe, current_drawdown):
    """Evaluate if the new parameters improve the strategy."""
    ...
    if sharpe_improved and drawdown_improved:
        return True, avg_profit_pct
    return False, 0.0


def commit_and_push(args, worst_strategy, strategy_json, avg_profit_pct):
    """Commit and push changes if improvements found."""
...
Examples:
  ...
  %(prog)s --yes
    ...
    strategy_json, backup_json = run_hyperopt_process(worst_strategy)
    if not strategy_json:
        sys.exit(1)

    success, avg_profit_pct = evaluate_results(worst_strategy, current_sharpe, current_drawdown)

    if success:
        print("Evaluation PASSED. Committing changes.")
        commit_and_push(args, worst_strategy, strategy_json, avg_profit_pct)

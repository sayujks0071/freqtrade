...
def read_backtest_result(filepath: Path) -> dict[str, Any] | None:
    data = None
...
def find_available_strategy() -> str | None:
...
def run_backtest_job(strategy_name: str) -> dict[str, Any] | None:
...
def ensure_baseline_backtest() -> dict[str, Any] | None:
...
def run_hyperopt_for_strategy(
    strategy_name: str,
) -> tuple[subprocess.CompletedProcess, Path, Path, bool]:
    """Run hyperopt for a given strategy."""
    ...
    if dry_run:
        ...
        if current_branch != target_branch:
            ...
            if result.returncode != 0:
                ...
                # Note: Logic to revert changes would need passed args or restructure,
                # handled by main mostly.
                sys.exit(1)
...
Examples:
  ...
  %(prog)s --yes
    ...
    backtest_data = ensure_baseline_backtest()
    ...
    result_hyperopt, strategy_json, backup_json, created_new = run_hyperopt_for_strategy(
        worst_strategy
    )
    ...
    if result_hyperopt.returncode != 0:
        ...
        if strategy_json.exists() and not created_new:
            shutil.move(str(backup_json), str(strategy_json))

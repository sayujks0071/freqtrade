...
def establish_baseline():
    ...
    Returns (worst_strategy, current_sharpe, current_drawdown) or raises exception.
    """
...
def run_hyperopt_execution(worst_strategy):
    """
    Runs hyperopt optimization for the given strategy.
    Returns: (strategy_json_path, backup_json_path, created_new_bool)
    """
...
def ensure_feature_branch(args):
...
def commit_and_push(args, worst_strategy, avg_profit_pct, strategy_json, backup_json, created_new):
    ...
    target_branch = ensure_feature_branch(args)
...
Examples:
  ...
  %(prog)s --yes
    ...
    passed, avg_profit_pct = evaluate_results(
        worst_strategy, current_sharpe, current_drawdown, strategy_json, backup_json, created_new
    )

...
def verify_improvement(worst_strategy, current_sharpe, current_drawdown):
...
def commit_and_push(args, worst_strategy, avg_profit_pct, strategy_json, backup_json):
    ...
    if args.dry_run:
        ...
        print(f"  Message: {msg}")
        print(f"  Branch: {get_target_branch(args)}")
    ...
    if not checkout_branch(target_branch):
        ...
        # strategy_json exists (checked by caller or previous steps)
        # But we need to handle revert logic carefully
        # Actually caller logic for failure handled revert.
        # Here we are in "success" but git failed.
        # Let's simple exit 1 and let user handle it?
        sys.exit(1)

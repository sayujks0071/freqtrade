...
def establish_baseline():
    """
    Establish a performance baseline by finding the latest backtest or running a new one.
    ...
    return find_worst_strategy(backtest_data)


def run_hyperopt_process(strategy_name, strategy_json, backup_json, created_new):
    """
...
def commit_and_push(strategy_name, avg_profit_pct, strategy_json, args):
    """
...
def run_evaluation(worst_strategy, current_sharpe, current_drawdown):
    """
    Run verification backtest and compare results.
    Returns (passed, avg_profit_pct).
    """
...
def main():
    ...
    if not run_hyperopt_process(worst_strategy, strategy_json, backup_json, created_new):
        sys.exit(1)

    # 3. Evaluation
    passed, avg_profit_pct = run_evaluation(worst_strategy, current_sharpe, current_drawdown)

    if passed:
        print("Evaluation PASSED. Committing changes.")
        if not commit_and_push(worst_strategy, avg_profit_pct, strategy_json, args):
            # If commit/push fails, revert
            print("Reverting changes due to git failure...")
            cleanup(strategy_json, backup_json, created_new)
            sys.exit(1)
...
def cleanup(strategy_json, backup_json, created_new):
    """Reverts changes if evaluation failed or if needed."""
    if not created_new:
        shutil.move(backup_json, strategy_json)
    else:
        if strategy_json.exists():

...
def establish_baseline():
    """
    Finds or creates a baseline backtest to determine which strategy needs optimization.
    """
    ...
    return worst_strategy, current_sharpe, current_stats


def run_hyperopt_session(strategy):
    """
    Runs hyperopt for the selected strategy and updates the strategy file.
    Returns True if new parameters were applied, False otherwise.
    """
...
def verify_improvement(strategy, current_sharpe, current_drawdown):
    """
    Runs a backtest with the new parameters and compares results against baseline.
    """
    print("Running verification backtest with new parameters...")
    new_backtest_data = run_backtest_job(strategy)

    if not new_backtest_data:
        print("Failed to run verification backtest.")
        return False, 0.0

    new_stats = new_backtest_data["strategy"][strategy]
...
def commit_and_push(strategy, avg_profit_pct, args):
    """
    Commits and pushes the optimized strategy.
    """
    strategy_json = STRATEGIES_DIR / f"{strategy}.json"
    ...
    if current_branch != target_branch:
        ...
        if result.returncode != 0:
            ...
            # Ideally we should revert the json file here too if we can't switch branch
            # But skipping complexity for now as user can manual fix
            sys.exit(1)
    ...
    if not args.yes:
        ...
        print("This will:")
        print(f"  - Push optimized strategy parameters for {strategy}")
...
Examples:
  ...
  %(prog)s --yes
    ...
    worst_strategy, current_sharpe, current_stats = establish_baseline()
    ...
    if not run_hyperopt_session(worst_strategy):
        sys.exit(1)

    success, avg_profit = verify_improvement(worst_strategy, current_sharpe, current_drawdown)

    if success:
        print("Evaluation PASSED. Committing changes.")
        commit_and_push(worst_strategy, avg_profit, args)
    else:
        print("Evaluation FAILED. Reverting changes.")
        strategy_json = STRATEGIES_DIR / f"{worst_strategy}.json"
        backup_json = strategy_json.with_suffix(".json.bak")

        # Check if backup exists (meaning we modified an existing file)
        if backup_json.exists():
            shutil.move(backup_json, strategy_json)
            print("Restored original strategy parameters.")
        else:

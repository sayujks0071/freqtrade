...
def ensure_branch(target_branch):
    """Creates and switches to feature branch if not already on it."""
    ...
    if current_branch != target_branch:
        ...
        if check_result.returncode == 0:
            ...
            return run_command(["git", "checkout", target_branch], capture=True)
        else:
            # Branch doesn't exist, create it
            return run_command(["git", "checkout", "-b", target_branch], capture=True)
    return subprocess.CompletedProcess(args=[], returncode=0)


def perform_git_push(target_branch, yes_flag):
    """Handles the push confirmation and execution."""
    if not yes_flag:
...
def handle_git_operations(
    strategy_json, worst_strategy, avg_profit_pct, args, created_new, backup_json
):
    ...
    result = ensure_branch(target_branch)
    ...
    perform_git_push(target_branch, args.yes)
...
def backup_strategy_file(strategy_json):
...
def restore_backup(strategy_json, backup_json, created_new):
    if not created_new and backup_json.exists():
        shutil.move(backup_json, strategy_json)
...
def main():
    ...
    print(f"Selected Strategy: {worst_strategy}, Sharpe: {current_sharpe}, DD: {current_drawdown}")

    strategy_json = STRATEGIES_DIR / f"{worst_strategy}.json"
    backup_json, created_new = backup_strategy_file(strategy_json)
    ...
    if result_hyperopt.returncode != 0:
        ...
        restore_backup(strategy_json, backup_json, created_new)
        sys.exit(1)
    ...
    print(f"New Sharpe: {new_sharpe}, New Drawdown: {new_drawdown}")

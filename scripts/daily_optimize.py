...
def prepare_feature_branch(target_branch):
...
def commit_and_push_changes(strategy_json, msg, target_branch, args):
    if args.dry_run:
    ...
    result = prepare_feature_branch(target_branch)
    if result.returncode != 0:
...
def get_baseline():
    ...
    backtest_data = read_backtest_result(latest_file) if latest_file else None
...
def run_hyperopt_process(worst_strategy):
...
def handle_failure(
    worst_strategy, new_sharpe, new_drawdown, sharpe_improved, drawdown_improved, args
):
...
def revert_files(strategy_json, backup_json, created_new):
    if not created_new:
...
def main():
    ...
    success, strategy_json, backup_json, created_new = run_hyperopt_process(worst_strategy)
    if not success:
        revert_files(strategy_json, backup_json, created_new)
        sys.exit(1)
    ...
    if sharpe_improved and drawdown_improved:
        ...
        commit_and_push_changes(strategy_json, msg, target_branch, args)

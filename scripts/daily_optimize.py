...
def confirm_push(target_branch, worst_strategy, backup_json):
    """Confirm with the user before pushing changes."""
...
def execute_git_push(target_branch, strategy_json, msg):
...
def git_push_workflow(
    ...
    # Prepare branch
    result = prepare_branch(target_branch)
    ...
    if not args.yes:
        if not confirm_push(target_branch, worst_strategy, backup_json):
            return

    execute_git_push(target_branch, strategy_json, msg)
...
Examples:
  ...
  %(prog)s --yes
    ...
    result_hyperopt, strategy_json, backup_json, created_new = run_hyperopt_execution(
        worst_strategy
    )

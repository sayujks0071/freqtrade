...
def run_hyperopt(strategy_name, strategy_json, created_new):
    ...
    return result_hyperopt.stdout


def verify_results(strategy_name, current_sharpe, current_drawdown):
    """Verify optimization results with a new backtest."""
...
def commit_and_push(strategy_name, profit_pct, strategy_json, args):
    ...
    if current_branch != target_branch:
        ...
        if result.returncode != 0:
            ...
            # We don't revert here because we are in a helper function
            # But the caller expects us to handle success/fail
            sys.exit(1)
...
Examples:
  ...
  %(prog)s --yes
    ...
    if not worst_strategy:
        print("No valid baseline or strategies found.")
        sys.exit(1)
    ...
    hyperopt_output = run_hyperopt(worst_strategy, strategy_json, created_new)
    if not hyperopt_output:
        sys.exit(1)
    ...
    # 3. Evaluation
    success, profit_pct = verify_results(worst_strategy, current_sharpe, current_drawdown)

    if success:
        print("Evaluation PASSED. Committing changes.")
        commit_and_push(worst_strategy, profit_pct, strategy_json, args)
    else:

...
def establish_baseline():
    """
    Establish the baseline performance by finding the worst performing strategy.
    Returns (worst_strategy, current_sharpe, current_stats)
    """
...
def run_hyperopt_process(worst_strategy, strategy_json, backup_json, created_new):
    """Run the hyperopt command and return output."""
...
def apply_hyperopt_results(result_stdout, strategy_json, backup_json, created_new):
    """Extract and write new params."""
    new_params = extract_hyperopt_params(result_stdout)
    if new_params:
        ...
        # Revert changes if we failed to extract params
        if strategy_json.exists() and not created_new:
            shutil.move(backup_json, strategy_json)
        elif created_new and strategy_json.exists():
            strategy_json.unlink()
...
def evaluate_results(worst_strategy, current_sharpe, current_drawdown):
    """Run backtest to verify improvements."""
    ...
    passed = sharpe_improved and drawdown_improved
    return passed, avg_profit_pct


def commit_and_push_changes(args, worst_strategy, avg_profit_pct, strategy_json):
...
Examples:
  ...
  %(prog)s --yes
    ...
    result_hyperopt = run_hyperopt_process(worst_strategy, strategy_json, backup_json, created_new)

    apply_hyperopt_results(result_hyperopt.stdout, strategy_json, backup_json, created_new)

    # 3. Evaluation
    passed, avg_profit_pct = evaluate_results(worst_strategy, current_sharpe, current_drawdown)

    if passed:
        print("Evaluation PASSED. Committing changes.")
        commit_and_push_changes(args, worst_strategy, avg_profit_pct, strategy_json)

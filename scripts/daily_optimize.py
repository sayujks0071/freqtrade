...
def establish_baseline():
    """Finds or creates a baseline backtest result."""
...
def evaluate_and_commit(
    strategy_name, strategy_json, backup_json, created_new, current_sharpe, current_stats, args
):
    """Runs verification backtest and commits if improved."""
    ...
    if sharpe_improved and drawdown_improved:
        ...
        commit_and_push(args, strategy_json, msg, backup_json)
...
def commit_and_push(args, strategy_json, msg, backup_json):
    if args.dry_run:
    ...
    if current_branch != target_branch:
        ...
        if result.returncode != 0:
            ...
            # We can't revert easily if we don't pass backup path here,
            # but usually logic handles revert before calling this or outside.
            sys.exit(1)
...
Examples:
  ...
  %(prog)s --yes
    ...
    strategy_json, backup_json, created_new = execute_hyperopt(worst_strategy)

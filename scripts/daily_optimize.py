...
def log_optimization_result(strategy, status, details):
    """
    Log the optimization result to optimization_log.txt
    """
    log_file = Path("optimization_log.txt")
    timestamp = datetime.now().isoformat()
    entry = f"{timestamp} | {strategy} | {status} | {details}\n"
    try:
...
def run_hyperopt_execution(worst_strategy):
    ...
    if result_hyperopt.returncode != 0:
        ...
        log_optimization_result(worst_strategy, "FAILED", "Hyperopt execution failed")
...
def evaluate_results(
    worst_strategy, current_sharpe, current_drawdown, strategy_json, backup_json, created_new
):
    ...
    if not new_backtest_data:
        ...
        log_optimization_result(worst_strategy, "FAILED", "Verification backtest failed")
        if strategy_json.exists() and not created_new:
            shutil.move(backup_json, strategy_json)
        elif created_new and strategy_json.exists():
            strategy_json.unlink()
    ...
    return sharpe_improved, drawdown_improved, new_sharpe, new_drawdown, avg_profit_pct


def push_changes(target_branch, strategy_json, msg, worst_strategy, backup_json, created_new, args):
...
def handle_success(
    ...
    log_optimization_result(
        worst_strategy,
        "SUCCESS",
        f"ROI: {avg_profit_pct:.2f}%, Sharpe: {new_sharpe:.4f}, Drawdown: {new_drawdown:.4f}",
    )
    ...
    if args.dry_run:
        ...
        push_changes(
            target_branch, strategy_json, msg, worst_strategy, backup_json, created_new, args
        )


def handle_failure(
    worst_strategy, sharpe_improved, drawdown_improved, strategy_json, backup_json, created_new
):
    print("Evaluation FAILED. Reverting changes.")
    log_optimization_result(
        worst_strategy,
        "FAILED",
        f"Sharpe Improved: {sharpe_improved}, Drawdown Improved: {drawdown_improved}",
    )
    if not created_new:
        shutil.move(backup_json, strategy_json)
    else:
...
Examples:
  ...
  %(prog)s --yes
    ...
    strategy_json, backup_json, created_new = run_hyperopt_execution(worst_strategy)

    sharpe_improved, drawdown_improved, new_sharpe, new_drawdown, avg_profit_pct = evaluate_results(
        worst_strategy, current_sharpe, current_drawdown, strategy_json, backup_json, created_new
    )

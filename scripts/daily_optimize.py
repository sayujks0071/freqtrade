...
def evaluate_results(worst_strategy, strategy_json, backup_json, created_new):
...
def switch_to_branch(target_branch, backup_json, strategy_json, created_new):
    ...
    check_result = subprocess.run(
        ["git", "rev-parse", "--verify", target_branch], capture_output=True, text=True, check=False
    )
...
def git_push_workflow(
    ...
    if args.dry_run:
        ...
        if not switch_to_branch(target_branch, backup_json, strategy_json, created_new):
            sys.exit(1)
...
Examples:
  ...
  %(prog)s --yes
    ...
    new_sharpe, new_drawdown, avg_profit_pct = evaluate_results(

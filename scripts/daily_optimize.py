...
def establish_baseline():
    """Find a baseline strategy performance or run a new backtest."""
...
def execute_hyperopt(worst_strategy):
    """Run hyperopt optimization for the selected strategy."""
...
def apply_new_params(result_hyperopt, strategy_json):
    """Parse and apply new parameters to the strategy file."""
...
def verify_results(worst_strategy):
    """Run verification backtest and return new stats."""
...
def setup_feature_branch(target_branch, strategy_json, backup_json, created_new):
    """Create and switch to the feature branch, or handle failure."""
...
def perform_push(target_branch, backup_json):
...
def commit_changes(
    ...
    target_branch = args.branch or f"optimize-{date_str}"
    ...
    if current_branch != target_branch:
        setup_feature_branch(target_branch, strategy_json, backup_json, created_new)
    ...
    perform_push(target_branch, backup_json)
...
def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Run optimization without committing"
    )
    parser.add_argument(
        "--branch",
        type=str,
    ...
    # 4. Evaluation
    new_sharpe, new_drawdown, avg_profit_pct = verify_results(worst_strategy)

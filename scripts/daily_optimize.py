...
def establish_baseline():
    """Establish baseline backtest metrics."""
...
def run_verification(worst_strategy, strategy_json, backup_json, created_new):
    """Verify the new parameters with a backtest."""
...
def ensure_feature_branch(args, target_branch):
...
def confirm_push(target_branch, worst_strategy):
...
def perform_git_push(target_branch):
...
def commit_and_push(
    ...
    if not ensure_feature_branch(args, target_branch):
...
def main():
    ...
    new_sharpe, new_drawdown, avg_profit_pct = run_verification(

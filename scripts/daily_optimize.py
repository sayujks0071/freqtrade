...
def push_to_git(args, strategy_json, msg):
    ...
    if not args.yes:
        ...
        if response not in ["y", "yes"]:
            ...
            return True  # Committed but not pushed is still a "success" for the script
    ...
    if result.returncode == 0:
        ...
        return True  # Committed but not pushed


def commit_and_push(args, strategy_json, backup_json, created_new, worst_strategy, avg_profit_pct):
    ...
    success = push_to_git(args, strategy_json, msg)
...
def main():
    ...
    if sharpe_improved and drawdown_improved:
        ...
        commit_and_push(
            args, strategy_json, backup_json, created_new, worst_strategy, avg_profit_pct
        )
    else:

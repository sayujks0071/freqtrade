...
Examples:
  ...
  %(prog)s --yes
    ...
    # Setup logging to optimization_log.txt
    log_path = USER_DATA_DIR / "optimization_log.txt"
    ...
    print(f"\n--- Optimization Run Started: {datetime.now().isoformat()} ---")
    ...
    try:
        ...
        if sharpe_improved and drawdown_improved:
            ...
            if args.dry_run:
                ...
                run_command(["git", "add", "-f", str(log_path)])
        ...
        # Commit log if not successful and not dry-run
        if not committed_success and not args.dry_run:
            print("Committing optimization log due to failure/no-change...")
            run_command(["git", "add", "-f", str(log_path)])
            ...
            if diff_res.returncode != 0:
                ...
                if args.yes:
                    ...
                    print("Log committed locally. Use 'git push' to sync.")

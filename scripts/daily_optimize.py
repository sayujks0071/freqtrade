...
def establish_baseline():
    """
    Establish a baseline for comparison by running a backtest on the latest data.
    """
...
def run_hyperopt(strategy_name):
    """
    Run hyperoptimization on the selected strategy.
    """
    ...
    return strategy_json, backup_json, created_new, result_hyperopt


def setup_git_branch(target_branch):
    """
...
def commit_and_push_changes(args, strategy_name, strategy_json, avg_profit_pct, backup_json):
    """
    ...
    if args.dry_run:
        ...
        print(f"  Message: {msg}")
        branch = args.branch or f"optimize-{datetime.now().strftime('%Y%m%d')}"
        ...
        if not setup_git_branch(target_branch):
            return False
...
def verify_strategy(worst_strategy, strategy_json, backup_json, created_new):
    """
    Run verification backtest with new parameters.
    """
    ...
    return new_backtest_data


def save_hyperopt_params(result_hyperopt, strategy_json, backup_json, created_new):
    """
    Extract and save hyperopt parameters.
    """
...
def main():
    ...
    strategy_json, backup_json, created_new, result_hyperopt = run_hyperopt(worst_strategy)

    # Apply new parameters
    save_hyperopt_params(result_hyperopt, strategy_json, backup_json, created_new)

    # 3. Evaluation (Verification Backtest)
    new_backtest_data = verify_strategy(worst_strategy, strategy_json, backup_json, created_new)
    ...
    if sharpe_improved and drawdown_improved:
        ...
        if not commit_and_push_changes(
            args, worst_strategy, strategy_json, avg_profit_pct, backup_json
        ):

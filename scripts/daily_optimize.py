...
def establish_baseline():
    """Establish a performance baseline by checking recent backtests or running a new one."""
...
def execute_hyperopt(strategy_name, strategy_json, backup_json, created_new):
    """Run hyperopt for the target strategy."""
...
def apply_new_params(strategy_json, result_hyperopt, backup_json, created_new):
...
def evaluate_results(strategy_name, current_sharpe, current_drawdown):
    ...
    return (sharpe_improved and drawdown_improved), avg_profit_pct, new_backtest_data


def commit_and_push(args, strategy_name, avg_profit_pct, strategy_json, backup_json):
    ...
    if current_branch != target_branch:
        ...
        if result.returncode != 0:
            ...
            # We assume created_new logic was handled before or backup exists
            # Ideally we should pass created_new here too but keeping it simple
...
def main():
    ...
    apply_new_params(strategy_json, result_hyperopt, backup_json, created_new)

    # 3. Evaluation
    success, avg_profit_pct, _ = evaluate_results(worst_strategy, current_sharpe, current_drawdown)

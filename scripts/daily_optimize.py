...
def execute_hyperopt(worst_strategy, strategy_json, backup_json, created_new):
...
def apply_new_params(hyperopt_stdout, strategy_json, backup_json, created_new):
    new_params = extract_hyperopt_params(hyperopt_stdout)
    if new_params:
...
def evaluate_results(
    ...
    if sharpe_improved and drawdown_improved:
        ...
        commit_and_push(args, msg, strategy_json, worst_strategy)
...
def commit_and_push(args, msg, strategy_json, worst_strategy):
    if args.dry_run:
        print("\n[DRY-RUN MODE] Would have committed and pushed:")
        print(f"  File: {strategy_json}")
        print(f"  Message: {msg}")
        ts = datetime.now().strftime("%Y%m%d")
        branch_name = args.branch if args.branch else f"optimize-{ts}"

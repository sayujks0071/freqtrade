...
class Sentinel:
    ...
    def get_rpc_balance(self):
        try:
            response = requests.get(
                f"{self.rpc_url}/balance", auth=self.auth, timeout=10
            )
            if response.status_code == 200:
                data = response.json()
                # Return total balance in stake currency (assuming USDT or similar total)
                # The endpoint returns a dict. 'total' is usually the key for total balance
                # across all currencies in stake currency equivalent.
    ...
    def update_history(self, current_balance, current_price):
        ...
        if current_balance is not None:
            ...
            # Remove old
            while self.balance_history and (
                now - self.balance_history[0][0] > timedelta(hours=1)
            ):
        ...
        if current_price is not None:
            ...
            while self.price_history and (
                now - self.price_history[0][0] > timedelta(hours=4)
            ):
    ...
    def trigger_emergency(self, reason):
        ...
        if self.webhook_url:
            try:
                ...
                requests.post(
                    self.webhook_url,
                    json={"content": f"CRITICAL ALERT: {reason}"},
                    timeout=10,
                )
            except Exception as e:
    ...
    def run(self):
        ...
        while not self.tripped:
            try:
                ...
                logger.info(
                    f"Status OK. Balance: {balance}, BTC: {price}, "
                    f"DD: {dd_val:.2%}, Drop: {crash_val:.2%}"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--rpc-url", default="http://127.0.0.1:8080/api/v1", help="Freqtrade RPC URL"
    )
    parser.add_argument("--rpc-user", default="freqtrader", help="RPC Username")
    parser.add_argument(
        "--rpc-password", default="SuperSecurePassword123!", help="RPC Password"
    )
    ...
    sentinel = Sentinel(
        ...
        args.rpc_password,
        args.webhook_url,
        args.interval,  # Fix: Changed from args.check_interval to args.interval
    )
    sentinel.run()

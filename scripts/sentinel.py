...
try:
    ...
    print("Error: ft_client not found. Ensure you are running from the repo root or ft_client is installed.")
    sys.exit(1)
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(project_root / "user_data/logs/sentinel.log") if (project_root / "user_data/logs").exists() else logging.StreamHandler(sys.stdout)
    ]
)
...
class Sentinel:
    def __init__(self, config_path, interval=300, dry_run=False, one_shot=False, panic_sell=False):
        ...
        if not self.config_path.exists():
            logger.warning(f"Config file {self.config_path} not found. Some settings might be missing.")
            self.config = {}
        else:
    ...
    def alert(self, message):
        ...
        if self.openclaw_url:
            try:
                if not self.dry_run:
                    requests.post(self.openclaw_url, json={"message": message, "priority": "high"})
                else:
    ...
    def check_drawdown(self, current_balance):
        ...
        self.state["balance_history"] = [
            x for x in self.state["balance_history"]
        ...
        while True:
            try:
                ...
                if self.state.get("triggered"):
                    self.log("Sentinel previously triggered. Waiting for manual reset. (Set 'triggered': false in state file to resume)")
                    if self.one_shot: break
                ...
                try:
                    ...
                    if balance_data:
                        ...
                        self.log(f"Current Balance: {current_balance:.2f} {balance_data.get('stake', 'Units')}")
                    else:
                    ...
                    # So we continue, but current_balance remains 0 (drawdown check will be skipped/invalid)
                    pass
...
if __name__ == "__main__":
    ...
    parser.add_argument("--panic-sell", help="Liquidate all positions on trigger", action="store_true")

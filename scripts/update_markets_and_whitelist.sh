...
echo "Current Whitelist: $WHITELIST_JSON"
if [ ! -f "$WHITELIST_JSON" ]; then
    echo "Warning: No previous whitelist found at $WHITELIST_JSON"
fi

# Run Validator
# If it fails (exit code != 0), we stop here due to set -e (or explicit check below)
# We capture exit code to print specific message
set +e

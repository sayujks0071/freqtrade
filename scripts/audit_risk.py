...
def _extract_stoploss_value(node: ast.Assign) -> float | None:
    ...
    if len(node.targets) == 1 and isinstance(node.targets[0], ast.Name) and node.targets[0].id == 'stoploss':
...
def check_strategy_file(filepath: Path) -> bool:
    ...
    try:
        ...
        # Collect modifications first to avoid changing lines while iterating?
        # Actually AST uses original line numbers. If we modify lines list, line numbers are still valid index if we don't insert/delete lines.
        # We only replace lines, so count remains same.
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Assign):
                ...
                if stoploss_val is not None:
                    ...
                    # Looser means allowing more loss. So stoploss < -0.10 (e.g. -0.15) is looser.
                    # Compliance requires stoploss >= -0.10.
                    if stoploss_val < -0.10:
                        ...
                        if lineno < len(lines):
                            ...
                            # Check if original line had comments? We lose them if we replace blindly.
                            # But enforcing strict stoploss is more important.
                            lines[lineno] = f"{indent}stoploss = -0.10"
                            modified = True

        if modified:
            with filepath.open('w') as f:
                # Join lines and add trailing newline if original file had it? Usually yes.
                f.write('\n'.join(lines) + '\n')
            return True
        return False
...
def main():
    ...
    # Check root level strategies if any (unlikely but user said "user_data/strategies/*.py")
    # Just stick to user_data/strategies as requested.

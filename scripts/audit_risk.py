...
def check_and_fix_strategy(filepath: Path) -> bool:
    ...
    # But stoploss can also be defined at module level (less common for strategies but possible if not inside class?)
    ...
    # This might catch false positives if 'stoploss' is used as a local variable, but in a strategy file,
    # top-level class attributes are the concern.
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Assign):
            ...
            if is_stoploss:
                # Check value
                # Value can be a number or a UnaryOp (negative number)
                ...
                if val is not None and isinstance(val, (int, float)):
                    ...
                    if val < STOPLOSS_LIMIT:
                        ...
                        if "#" in original_line:
                            ...
                            new_line = f"{indent_str}stoploss = {STOPLOSS_LIMIT}  # {comment.strip()} (Fixed by Audit)"
                        else:
...
def main():
    ...
    # 2. Check strategies
    strategy_files = list(root.glob("user_data/strategies/*.py"))
    # Also check recursively? The prompt says "user_data/strategies/*.py" which usually implies top level,
    # but strictly "strategies/*.py" could mean recursive if using glob("**/*.py").
    # Given the previous ls, there is _base/AuditedStrategyMixin.py.
